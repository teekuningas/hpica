...
# S += 0.2 * np.random.RandomState(random_state).normal(size=S.shape)
# S = (S - S.mean(axis=0)) / S.std(axis=0)
...
for idx in range(n_subjects):
    ...
    # padded_template_s1 = np.pad(template, pad_width=[(1,1), (4, n_subjects+1)])

    padded_template_s1 = np.pad(template, pad_width=[(1,1),(idx+7, n_subjects+3-idx)])
    padded_template_s2 = np.pad(template, pad_width=[(1,1), (idx+4, n_subjects+6-idx)])
    padded_template_s3 = np.pad(template, pad_width=[(1,1), (0, n_subjects+10)])
    ...
    subject_s1 = s1.copy()
    subject_s2 = np.roll(s2, int(idx*n_samples/10))
    subject_s3 = s3.copy()
...
def plot_scores(sources, title):
    ...
    for subject_idx in range(n_subjects):
        ...
        for unmixed_ts in sources[subject_idx].T:
            ...
            subject_results.append(np.max(coefs))
